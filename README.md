...
- **Liquidity is locked from the first block.** The launch position belongs to the hook. The hook adds liquidity once, at launch, and afterwards only collects swap fees; it has no function that decreases or withdraws that liquidity, and no LP token exists. Before a coin is bonded nobody else can add liquidity to the pool. Check any coin two ways: the live proof page **`https://arcstockpad.com/proof/<token>`** (reads the chain in your own browser and prints the exact `eth_call`), or directly — Uniswap V4 StateView `getPositionInfo(poolId, hook, tickLower, tickUpper, 0x0)` against `getLiquidity(poolId)`. See [the integration guide](docs/integration-guide.md#verifying-the-liquidity-lock-for-lp-locked-badges) for the tick ranges per build.
